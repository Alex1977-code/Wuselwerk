...
const ZIEL = args.includes('--ziel') ? args[args.indexOf('--ziel') + 1] : 'src/art';
...
const figurName = args.find((a) => !a.startsWith('--') && !/^[\d.]+$/.test(a) && !a.includes('/'))
  ?? 'erdmaennchen';
const QUELLE = args.includes('--quelle') ? args[args.indexOf('--quelle') + 1] : join('art-src', figurName);
...
const zahl = (f, v) => (args.includes(f) ? Number(args[args.indexOf(f) + 1]) : v);
...
if (probe) {
  ...
  writeFileSync(`${ZIEL}/${probeName}.webp`, daten);
