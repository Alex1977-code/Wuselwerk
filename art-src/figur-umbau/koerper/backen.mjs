...
const ZIEL = process.env.WW_ZIEL || 'art-src/proben/figur2/koerper/blatt';
...
const zahl = (flagge, vor) => (args.includes(flagge) ? Number(args[args.indexOf(flagge) + 1]) : vor);
...
const GLB = args.includes('--modell')
  ? args[args.indexOf('--modell') + 1]
  : join(QUELLE, eigenschaften.modell);
...
// --- Probe koerper: drei Regler ---------------------------------------
// KOPF   Massstab des Kopfknochens (Haar haengt daran). <1 = kleinerer Kopf.
// SCHMAL waagerechte Stauchung der ganzen Figur (Links-Rechts-Achse).
// TIEF   dasselbe in der Tiefe (Vorn-Hinten). Beide zusammen = duennere Figur.
const KOPF = zahl('--kopf', eigenschaften.kopfSkala ?? 1);
const SCHMAL = zahl('--schmal', 1);
const TIEF = zahl('--tief', zahl('--schmal', 1));
const SICHT = 1.22;
const FUSS_PX = 3;
/**
