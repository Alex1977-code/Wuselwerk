...
/*
 * Hier standen die gemessenen Haartoene und zwei Zeichner: `drawHaarZacken`
 * und `drawHaarStraehnen`, zusammen rund zweihundertsechzig Zeilen, die dem
 * gebackenen Kopf zur Laufzeit Zacken und Straehnen aufmalten.
 *
 * Sie sind ersatzlos entfallen — nicht, weil sie schlecht gebaut waren,
 * sondern weil sie an der falschen Stelle standen. Ein Zeichner, der nur die
 * fertige Blattzelle kennt, kann Tinte hinzufuegen, aber der grosse Teil
 * davon landet INNERHALB der vorhandenen Haarkuppel, weil dort schon Haar
 * ist. Gemessen trug der ganze Zackenkamm in der Frontansicht 4,2 Prozent
 * neue Flaeche bei: sechs Bildpunkte bei Telefongroesse. Umriss kauft man
 * nicht durch Hinzufuegen, sondern durch Amplitude — und die entscheidet die
 * Geometrie, nicht der Pinsel.
 *
 * Die Form sitzt seit dem Haar-Umbau im Modell (`scripts/haar-bauen.mjs`):
 * Dort wird das vorhandene Lappenrelief der Haarschale gespreizt, und der
 * Kletterruck haengt am Knochen `HaarSchwung` und ist in die Bilder gebacken.
 *
 * Was hier bleibt, ist `drawBand`. Die acht Berufsfarben muessen zur Laufzeit
 * umschaltbar sein — ein Bogen je Figur ist dafuer der richtige Preis, und
 * ein gebackenes Band waere acht Blattzeilen teuer.
 */
