...
// ===========================================================================
// 2a. Die Ableitungen: aus einem abgenommenen Takt werden mehrere
// ===========================================================================
//
...
//    Damit koennen B4 und die 3-kHz-Decke gar nicht erst reissen.
...
function stufeVon(h: number, leiter: readonly number[]): number | null {
...
function tonVon(s: number, leiter: readonly number[]): number {
  const okt = Math.floor(s / 7);
  return okt * 12 + leiter[s - okt * 7];
}

type Ableitung = { name: string; art: 'id' | 'seq' | 'spiegel'; d: number };

/** Eintrag 0 ist die Urgestalt — daran haengt "Level 1 ist das Weltstueck". */
const ABLEITUNGEN: readonly Ableitung[] = [
  { name: '=', art: 'id', d: 0 },
  { name: '+1', art: 'seq', d: 1 },
  { name: '-1', art: 'seq', d: -1 },
...
function leite(b: Takt, op: Ableitung, leiter: readonly number[]): Takt | null {
  if (op.art === 'id') return kopie(b);
  const stufen: (number | null)[] = [];
  for (const n of b) {
    if (n[0] === null) { stufen.push(null); continue; }
  ...
  for (let i = 0; i < b.length; i++) {
    const s = stufen[i];
    if (s === null) { raus.push([null, b[i][1]]); continue; }
...
function leiteOderUr(b: readonly Note[], iOp: number, leiter: readonly number[]): Takt {
  return leite(kopie(b as Takt), ABLEITUNGEN[iOp], leiter) ?? kopie(b as Takt);
}
...
function baueTakte(
  ...
  for (const slot of form.slots) {
    if (slot === 'K') {
      ...
      const beide = paar.every((b) => leite(kopie(b as Takt), ABLEITUNGEN[opM], f.leiter) !== null);
      bars.push(beide ? leiteOderUr(paar[mTeil], opM, f.leiter) : kopie(paar[mTeil] as Takt));
      mTeil++;
    } else if (slot === 'L') {
 ...
 * auf null stehen (der Fehler, den Weg A zweite Fassung gemeldet hat).
 */
...
export function stueckFuer(levelId: string, theme: string): Stueck {
  ...
  return montiere(theme, [
    ...
    k % WELTEN[theme].zweitStimmen.length,
    k % WELTEN[theme].harmonieStimmen.length,
