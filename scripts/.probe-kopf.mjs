...
// ===========================================================================
// ANHANG 2 (Gegenprobe): Kopf-Nahaufnahme mit den INNEREN Groessen
// ===========================================================================
...
const WUNSCH = (process.env.ZEIGEN ?? 'walking:0,dying:3,spaehen:0,saving:5').split(',');
const Z = 10; // Zoom

const idx = WUNSCH.map((w) => {
  const [clip, bild] = w.split(':');
  const i = BILDER.findIndex((b) => b.clip === clip && b.bild === Number(bild));
  if (i < 0) throw new Error('Kein Bild ' + w);
  return i;
});

const n = ZELLE;
// Ausschnitt: Kopfbereich
const AUSSCHNITT = [];
for (const bi of idx) {
  const z = zerlegen(felder[bi].blatt);
  const hk = kasten(z.haar, n);
  const hi = inseln(z.haut, n);
  const g = new Uint8Array(n * n);
  for (const i of hi[0] ?? []) g[i] = 1;
  const gk = kasten(g, n);
  const x0 = Math.max(0, Math.min(hk.x0, gk.x0) - 2);
  const y0 = Math.max(0, Math.min(hk.y0, gk.y0) - 2);
  const x1 = Math.min(n - 1, Math.max(hk.x1, gk.x1) + 2);
  const y1 = Math.min(n - 1, Math.max(hk.y1, gk.y1) + 2);
  AUSSCHNITT.push({ bi, x0, y0, w: x1 - x0 + 1, h: y1 - y0 + 1 });
}
const CW = Math.max(...AUSSCHNITT.map((a) => a.w));
const CH = Math.max(...AUSSCHNITT.map((a) => a.h));
const SP = 3;
const breite = CW * Z * SP;
const hoehe = CH * Z * AUSSCHNITT.length;
const out = Buffer.alloc(breite * hoehe * 4, 0);
const setz = (px, py, r, g, b) => {
  if (px < 0 || py < 0 || px >= breite || py >= hoehe) return;
  const o = (py * breite + px) * 4;
  out[o] = r; out[o + 1] = g; out[o + 2] = b; out[o + 3] = 255;
};

AUSSCHNITT.forEach((a, reihe) => {
  const f = felder[a.bi].blatt;
  const p = f.p;
  const z = zerlegen(f);
  const m = vermessen(z);
  const hi = inseln(z.haut, n);
  const ges = new Uint8Array(n * n);
  for (const i of hi[0] ?? []) ges[i] = 1;
  const gVoll = mitLoechern(ges, n);
  const gk = kasten(ges, n);

  const grund = (x, y) => {
    const i = (y * n + x) * 4;
    const al = p[i + 3] / 255;
    const k = ((x >> 2) + (y >> 2)) & 1 ? 70 : 50;
    return [p[i] * al + k * (1 - al), p[i + 1] * al + k * (1 - al), p[i + 2] * al + k * (1 - al)];
  };
  const zeichne = (spalte, mal) => {
    for (let yy = 0; yy < a.h; yy++) {
      for (let xx = 0; xx < a.w; xx++) {
        const c = mal(a.x0 + xx, a.y0 + yy);
        for (let dy = 0; dy < Z; dy++) {
          for (let dx = 0; dx < Z; dx++) {
            setz(spalte * CW * Z + xx * Z + dx, reihe * CH * Z + yy * Z + dy, c[0], c[1], c[2]);
          }
        }
      }
    }
  };
  const wlinie = (spalte, yb, r, g, b, dick = 2) => {
    const py = Math.round(reihe * CH * Z + (yb - a.y0) * Z);
    for (let px = spalte * CW * Z; px < (spalte + 1) * CW * Z; px++) {
      for (let d = 0; d < dick; d++) setz(px, py + d, r, g, b);
    }
  };
  const slinie = (spalte, xb, r, g, b) => {
    const px = Math.round(spalte * CW * Z + (xb - a.x0) * Z);
    for (let py = reihe * CH * Z; py < (reihe + 1) * CH * Z; py++) setz(px, py, r, g, b);
  };

  zeichne(0, grund);
  // Spalte 1: Haarrand als Kontur ueber dem Original
  zeichne(1, (x, y) => {
    const k = y * n + x;
    const c = grund(x, y);
    const rand = z.haar[k] && [[1, 0], [-1, 0], [0, 1], [0, -1]].some(([dx, dy]) => {
      const nx = x + dx, ny = y + dy;
      return nx < 0 || ny < 0 || nx >= n || ny >= n || !z.haar[ny * n + nx];
    });
    if (rand) return [255, 0, 220];
    if (z.splitterMaske[k]) return [0, 255, 60];
    return c;
  });
  // Spalte 2: Augen-/Stirndiagnose
  const dunkelSet = new Set((m._dunkel ?? []).map(([x, y]) => y * n + x));
  const kanteSet = new Map((m._kante ?? []).map((k) => [k.x, k.y]));
  const kanteAlleSet = new Map((m._kanteAlle ?? []).map((k) => [k.x, k.y]));
  zeichne(2, (x, y) => {
    const k = y * n + x;
    const c = grund(x, y).map((v) => v * 0.55 + 40);
    if (dunkelSet.has(k)) return [255, 40, 40];
    if (kanteSet.get(x) === y + 0.5) return [255, 255, 0];
    else if (kanteAlleSet.get(x) === y + 0.5) return [255, 140, 0];
    if (z.haar[k]) return [c[0] * 0.5, c[1] * 0.5, c[2]];
    if (gVoll[k] && !ges[k]) return [0, 150, 255];
    return c;
  });
  if (gk) {
    for (const sp of [1, 2]) {
      slinie(sp, gk.x0 + gk.w / 3, 0, 255, 255);
      slinie(sp, gk.x0 + (2 * gk.w) / 3, 0, 255, 255);
    }
    wlinie(2, gk.y0, 255, 255, 0, 1);
    wlinie(2, gk.y1 + 1, 255, 255, 0, 1);
    wlinie(2, gk.y0 + gk.h * 0.65, 150, 150, 255, 1);
    if (m.augenzeile !== null) wlinie(2, m.augenzeile, 255, 0, 0);
    if (m.haartiefe !== null) wlinie(2, m.haartiefe, 0, 255, 0);
    if (m.augenzeile !== null) wlinie(1, m.augenzeile, 255, 0, 0);
    if (m.haartiefe !== null) wlinie(1, m.haartiefe, 0, 255, 0);
  }
  console.log(
    `${(BILDER[a.bi].clip + '[' + BILDER[a.bi].bild + ']').padEnd(13)}` +
      ` gesicht=${gk.x0},${gk.y0} ${gk.w}x${gk.h}` +
      ` drittel=[${(gk.x0 + gk.w / 3).toFixed(1)},${(gk.x0 + 2 * gk.w / 3).toFixed(1)}]` +
      ` augenzeile=${m.augenzeile} gipfelZeile=${m._gipfelZeile} dunkelPunkte=${(m._dunkel ?? []).length}` +
      ` haartiefe=${m.haartiefe} schranke=${m.augenschranke}` +
      ` kante=${(m._kante ?? []).length}/${(m._kanteAlle ?? []).length} stirnwechsel=${m.stirnwechsel}`,
  );
});
writeFileSync(AUS + 'kopf.png', pngVon(out, breite, hoehe));
console.log('PNG: ' + AUS + 'kopf.png');
console.log('Spalten: 1 Original | 2 Haarrand magenta, Splitter gruen | 3 Diagnose: rot=Augenpunkte, gelb=Stirnstrecke, orange=verworfene Kante, cyan=mittleres Drittel, rote Linie=Augenzeile, gruen=tiefstes Haar');
