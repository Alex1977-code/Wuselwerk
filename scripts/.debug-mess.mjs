...
const WURZEL = '/home/user/Wuselwerk/';
...
const ZIEL = 'art-src/wuselwerker/blatt-mess-DEBUG.json';
...
// ===========================================================================
// ANHANG (nur Gegenprobe): Erkannte Flaechen einfaerben und als PNG ausgeben
// ===========================================================================
...
const WUNSCH = (process.env.ZEIGEN ?? 'walking:0,saving:5,climbing:1,dying:3,spaehen:0,building:2').split(',');
const SKAL = 4;

function tafel(bilderIdx, name) {
  const spalten = 4;
  const n = ZELLE;
  const B = n * SKAL;
  const breite = B * spalten;
  const hoehe = B * bilderIdx.length;
  ...
  const block = (spalte, reihe, mal) => {
    for (let y = 0; y < n; y++) {
      for (let x = 0; x < n; x++) {
        const c = mal(x, y);
        for (let dy = 0; dy < SKAL; dy++) {
          for (let dx = 0; dx < SKAL; dx++) {
            setz(spalte * B + x * SKAL + dx, reihe * B + y * SKAL + dy, c[0], c[1], c[2]);
          }
        }
      }
    }
  };
  // Linie in Blockkoordinaten (Subpixel), waagerecht
  const linie = (spalte, reihe, yy, r, g, b) => {
    const py = Math.round(reihe * B + yy * SKAL);
    for (let px = spalte * B; px < (spalte + 1) * B; px++) { setz(px, py, r, g, b); setz(px, py + 1, r, g, b); }
  };
  const senk = (spalte, reihe, xx, r, g, b) => {
    const px = Math.round(spalte * B + xx * SKAL);
    for (let py = reihe * B; py < (reihe + 1) * B; py++) { setz(px, py, r, g, b); }
  };

  bilderIdx.forEach((bi, reihe) => {
    const f = felder[bi].blatt;
    const p = f.p;
    const z = zerlegen(f);
    const m = vermessen(z);
    const karo = (x, y) => (((x >> 3) + (y >> 3)) & 1 ? 60 : 40);
    const grund = (x, y) => {
      const i = (y * n + x) * 4;
      const a = p[i + 3] / 255;
      const k = karo(x, y);
      return [p[i] * a + k * (1 - a), p[i + 1] * a + k * (1 - a), p[i + 2] * a + k * (1 - a)];
    };
    // Spalte 0: Original
    block(0, reihe, grund);
    // Spalte 1: Haarflaeche (grosse Inseln) magenta, Splitter gruen
    block(1, reihe, (x, y) => {
      const k = y * n + x;
      const g = grund(x, y).map((v) => v * 0.35);
      if (z.haar[k]) return [255, 0, 220];
      if (z.splitterMaske[k]) return [0, 255, 60];
      return g;
    });
    // Spalte 2: alle Klassen
    block(2, reihe, (x, y) => {
      const k = y * n + x;
      if (z.haar[k] || z.splitterMaske[k]) return [255, 0, 220];
      if (z.haut[k]) return [255, 200, 120];
      if (z.tunika[k]) return [120, 220, 120];
      if (z.deckend[k]) return [200, 200, 200];
      return [30, 30, 30];
    });
    // Spalte 3: Gesichtsdiagnose
    const hautInseln = inseln(z.haut, n);
    const gi = hautInseln[0] ?? [];
    const gesicht = new Uint8Array(n * n);
    for (const i of gi) gesicht[i] = 1;
    const gVoll = mitLoechern(gesicht, n);
    const gk = kasten(gesicht, n);
    block(3, reihe, (x, y) => {
      const k = y * n + x;
      const g = grund(x, y).map((v) => v * 0.4);
      if (z.haar[k]) return [180, 0, 160];
      if (gVoll[k] && !gesicht[k]) return [0, 160, 255];
      if (gesicht[k]) return [255, 190, 110];
      return g;
    });
    if (gk) {
      // Gesichtskasten
      linie(3, reihe, gk.y0, 255, 255, 0);
      linie(3, reihe, gk.y1 + 1, 255, 255, 0);
      senk(3, reihe, gk.x0, 255, 255, 0);
      senk(3, reihe, gk.x1 + 1, 255, 255, 0);
      // mittleres Drittel
      senk(3, reihe, gk.x0 + gk.w / 3, 0, 255, 255);
      senk(3, reihe, gk.x0 + (2 * gk.w) / 3, 0, 255, 255);
      // Suchgrenze fuer Augen
      linie(3, reihe, gk.y0 + gk.h * 0.65, 120, 120, 255);
      if (m.augenzeile !== null) linie(3, reihe, m.augenzeile, 255, 0, 0);
      if (m.haartiefe !== null) linie(3, reihe, m.haartiefe, 0, 255, 0);
    }
    console.log(
      `${String(BILDER[bi].clip + '[' + BILDER[bi].bild + ']').padEnd(14)}` +
        ` rauhBlatt=${m.rauheitBlatt?.toFixed(3)} haarVorHaut=${m.haarVorHaut}` +
        ` splitterVorHaut=${m.splitterVorHaut} inseln=${z.inselzahl} splitter=${z.splitter}` +
        ` augenzeile=${m.augenzeile} haartiefe=${m.haartiefe} schranke=${m.augenschranke}` +
        ` gesichtsbreite=${m.gesichtsbreite} gk=${gk ? `${gk.x0},${gk.y0} ${gk.w}x${gk.h}` : '--'}` +
        ` hautInseln=${hautInseln.length} groesse0=${hautInseln[0]?.length} groesse1=${hautInseln[1]?.length}`,
    );
  });
  require$writeFileSync(AUS + name, pngVon(out, breite, hoehe));
  console.log('PNG: ' + AUS + name);
}
const require$writeFileSync = writeFileSync;
...
console.log('\n=== Sichtprobe ===');
console.log('Spalten: 1 Original | 2 Haarflaeche magenta / Splitter gruen | 3 alle Klassen | 4 Gesichtsdiagnose');
tafel(idx, 'sicht.png');
